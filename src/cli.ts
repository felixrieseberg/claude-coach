...
async function runAuth(args: AuthArgs): Promise<void> {
  ...
  if (args.code) {
    ...
    if (code.includes("localhost") || code.startsWith("http")) {
      ...
      if (url) {
        ...
        // The redirect must belong to the authorization URL this tool printed;
        // otherwise someone else's code (and Strava account) could be slipped in.
        if (
          config.pending_auth_state &&
          !authStateMatches(url.searchParams.get("state"), config.pending_auth_state)
        ) {
          log.error(
            "This redirect URL does not belong to the authorization link generated by this tool."
          );
          log.info("Run 'auth --client-id=... --client-secret=...' again and use the new link.");
          process.exit(1);
        }
        code = extractedCode;
      }
    ...
    saveTokens(tokens);
    if (config.pending_auth_state) {
      delete config.pending_auth_state;
      saveConfig(config);
    }
  ...
  // Save config for later use, together with the state that the redirect URL
  // must echo back in the --code step.
  const state = createAuthState();
  const config = createConfig(args.clientId, args.clientSecret, 730);
  config.pending_auth_state = state;
  saveConfig(config);

  const authUrl = buildAuthorizeUrl(args.clientId, state);
